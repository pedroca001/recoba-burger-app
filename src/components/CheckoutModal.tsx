...
export function CheckoutModal({ open, items, total, address, config, onClose, onPaid }: Props) {
  ...
  const validate = () => {
    ...
    if (!/^\S+@\S+\.\S+$/.test(customer.email)) return "Digite um e-mail válido.";
    if (digitsOnly(customer.phone).length < 10) return "Digite um telefone com DDD.";
    if (digitsOnly(customer.document).length !== 11) return "Digite um CPF com 11 números.";
  ...
  return (
    <div className="modal-backdrop checkout-backdrop" role="dialog" aria-modal="true" aria-labelledby="checkout-title">
      <div className="checkout-modal">
        <div className="checkout-header"><div><span>Pagamento seguro</span><h2 id="checkout-title">Finalizar pedido</h2></div><button type="button" onClick={onClose}><X /></button></div>
        <form onSubmit={submit}>
          <div className="checkout-scroll">
            <section className="checkout-section">
              <h3>Seus dados</h3>
              <div className="form-grid">
                <label className="full">Nome completo<input value={customer.name} onChange={(e) => updateCustomer("name", e.target.value)} autoComplete="name" /></label>
                <label className="full">E-mail<input type="email" value={customer.email} onChange={(e) => updateCustomer("email", e.target.value)} autoComplete="email" /></label>
                <label>Celular com DDD<input value={customer.phone} onChange={(e) => updateCustomer("phone", e.target.value)} inputMode="tel" autoComplete="tel" placeholder="(11) 99999-9999" /></label>
                <label>CPF<input value={customer.document} onChange={(e) => updateCustomer("document", e.target.value)} inputMode="numeric" placeholder="000.000.000-00" /></label>
              </div>
            </section>
            ...
            <section className="checkout-section delivery-section">
              ...
              <p className="confirmed-address">{address.normalizedAddress}</p>
              <div className="form-grid">
                ...
                <label className="full">Ponto de referência<input value={customer.reference} onChange={(e) => updateCustomer("reference", e.target.value)} /></label>
              </div>
            </section>
